...
logs = os.path.join(root, 'perf/e2e_offline_val_rqssensitivity')
...
def plot_figure(df):
    ...
    plt.savefig(os.path.join(root, "perf/fig/e2e_offline/offline_rqs_sensitivity.pdf"))
...
def read_perf_record(path):
    ...
    pdr = prettify_attn_name(get_substring(path, '_pdr_', '_rqs_'))
    cl = prettify_attn_name(get_substring(path, '_cl_', '_sarathi'))
    rqs = prettify_attn_name(get_substring(path, '_rqs_', '_cl_'))
    df = pd.read_csv(path)

    latency = df['request_e2e_time'].max()
    num_requests = len(df)
    if rqs not in record:
        record[rqs] = {}
    if attn not in record[rqs]:
        record[rqs][attn] = (num_requests * 60) / latency
...
pdrs = ['256', '512', '1024', '2048']
methods = ['FA_Serial', 'POD_Attn','Load_Attn']
for pdr in pdrs:
    if pdr not in df.index:
        df.loc[pdr] = 0
    for attn in methods:
        if attn not in df.loc[pdr]:
            df.loc[pdr, attn] = 0
