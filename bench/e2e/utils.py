...
offline_experiment_dir = os.path.join(root, 'perf', 'e2e_offline_val')
online_experiment_dir = os.path.join(root, 'perf', 'e2e_online_val')
...
models = {
    'yi-6b-1': {'tp': 1, 'hfrecord': '../01ai/Yi-6B-200K', 'logentry': 'yi-6b', 'pd_ratio': '18', 'chunk_size' : 512, 'num_req' : 256},
    'llama-3-8b-1': {'tp': 1, 'hfrecord': '../LLM-Research/Meta-Llama-3-8B' , 'logentry': 'llama-3-8b', 'pd_ratio': '24', 'chunk_size' : 1024, 'num_req' : 256},
    'llama-2-7b-1': {'tp': 1, 'hfrecord': '../shakechen/Llama-2-7b-hf' , 'logentry': 'llama-2-7b', 'pd_ratio': '64', 'chunk_size' : 1024, 'num_req' : 1024},
    'progen2-1': {'tp': 1, 'hfrecord': '../InstructPLM/MPNN-ProGen2-xlarge-CATH42' , 'logentry': 'progen2', 'pd_ratio': '64', 'chunk_size' : 512, 'num_req' : 512},
    'llama-13b-1': {'tp': 1, 'hfrecord': '../modelscope/Llama-2-13b-ms' , 'logentry': 'llama-13b', 'pd_ratio': '96', 'chunk_size' : 1024, 'num_req' : 256},
    'mistral-7b-1': {'tp': 1, 'hfrecord': '../mistralai/Mistral-7B-v0___1' , 'logentry': 'mistral-7b', 'pd_ratio': '24', 'chunk_size' : 1024, 'num_req' : 256},
    'mixtral-8x7b-1': {'tp': 1, 'hfrecord': '../AI-ModelScope/Mixtral-8x7B-v0___1' , 'logentry': 'mixtral-8x7b', 'pd_ratio': '24', 'chunk_size' : 1024, 'num_req' : 256},
    ...
    # 'qwen2.5-7b-1': {'tp': 1, 'hfrecord': '../Qwen/Qwen2___5-7B-Instruct', 'logentry': 'qwen2__5-7b'}
}
...
def get_block_or_page_size(attn_backend):
    if '64kb' in attn_backend.lower():
        ...
        return 2 * MB
    elif 'paged' in attn_backend.lower():
...
def get_backend(attn_backend):
    ...
    elif 'fa_pod_paged' in attn_backend.lower():
        return 'fa_pod_paged'
    elif 'fa_pod_sparse' in attn_backend.lower():
        return 'fa_pod_sparse'
    elif 'fa_pod' in attn_backend.lower():
        return 'fa_pod'
    elif 'fa_paged' in attn_backend.lower():
        return 'fa_paged'
    elif 'fi_paged' in attn_backend.lower():
        return 'fi_paged'
    elif 'la_paged' in attn_backend.lower():
        return 'la_paged'
    elif 'fa_streams_paged' in attn_backend.lower():
        return 'fa_streams_paged'
    elif 'hfuse_vattn' in attn_backend.lower():
        return 'hfuse_vattn'
    elif 'hfuse_sparse' in attn_backend.lower():
        return 'hfuse_sparse'
    elif 'torch_paged' in attn_backend.lower():
        return 'torch_paged'
    elif 'fa_sparse' in attn_backend.lower():
        return 'fa_sparse'
    elif 'la_sparse' in attn_backend.lower():
        return 'la_sparse'
    else:
        raise ValueError(f"Unsupported attention backend: {attn_backend}")
