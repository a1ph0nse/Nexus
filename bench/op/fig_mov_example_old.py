...
def plot_sm_cycles():
    fig = plt.figure(figsize=(16, 6))  # 单图尺寸，可按需调整
    ax = fig.add_subplot(111)
    font_size = 24
    # 配置参数
    model_list = sorted(list(set(k[0] for k in record.keys())))
    width = 0.15
    x = np.arange(len(model_list)) * 0.7  # 保持你要的横坐标间距
    colors = {
        'POD_min': '#2E86AB',
        'POD_max': '#549CD6',
        'HFuse_min': '#C73E1D',
    ...
    # 样式设置
    ax.set_xticks(x)
    ax.set_xticklabels(model_list, fontsize=font_size, rotation=0, ha='center')
    ax.set_ylabel('SM Active Cycles', fontsize=font_size+4)
...
def plot_mem_stall():
    fig = plt.figure(figsize=(16, 12))  # 单图尺寸，可按需调整
    ax = fig.add_subplot(111)
    font_size = 48
    # 配置参数
    model_list = sorted(list(set(k[0] for k in record.keys())))
    ...
    # 绘制柱子
    ax.bar(x - 1.5*width, pod_stall_sum, width, color=colors['POD_max'], edgecolor='black', linewidth=0.8)
    ...
    ax.set_xticks(x)
    ax.set_xticklabels(model_list, fontsize=font_size, rotation=0, ha='center')
    ax.set_ylabel('Stall Cycle', fontsize=font_size+4)
    ax.grid(axis='y', linestyle='--', alpha=0.3)
    ax.set_ylim(top=ax.get_ylim()[1] * 1.15)
    ax.ticklabel_format(axis='y', style='sci', scilimits=(0,0), useMathText=True)
    ...
    # 图例（单图专属，放在图内更美观）
    handles = [
        plt.Rectangle((0,0),1,1, color=colors['POD_max'], label='POD total stall'),
        plt.Rectangle((0,0),1,1, color=colors['POD_min'], label='POD mem stall'),
...
def plot_memory_stall():
    ...
    # 绘制柱子
    ax.bar(x - width/2, pod_vals, width, color=colors['POD'], edgecolor='black', linewidth=0.8)
    ax.bar(x + width/2, hf_vals,  width, color=colors['HFuse'], edgecolor='black', linewidth=0.8)

    # 样式设置
    ax.set_xticks(x)
    ax.set_xticklabels(model_list, fontsize=font_size, rotation=0, ha='center')
    ax.set_ylabel('Memory Access Stall Ratio', fontsize=font_size+4)
    ax.grid(axis='y', linestyle='--', alpha=0.3)
    ax.set_ylim(top=ax.get_ylim()[1] * 1.15)
    ax.ticklabel_format(axis='y', style='sci', scilimits=(0,0), useMathText=True)
    ...
    # 图例
    handles = [
        plt.Rectangle((0,0),1,1, color=colors['POD'], label='POD'),
...
def plot_warp_instruction():
    ...
    ax.set_xticks(x)
    ax.set_xticklabels(model_list, fontsize=font_size, rotation=0, ha='center')
    ax.set_ylabel('Warp Instruction Executed', fontsize=font_size+4)
    ax.grid(axis='y', linestyle='--', alpha=0.3)
    ax.set_ylim(top=ax.get_ylim()[1] * 1.15)
    ax.ticklabel_format(axis='y', style='sci', scilimits=(0,0), useMathText=True)
