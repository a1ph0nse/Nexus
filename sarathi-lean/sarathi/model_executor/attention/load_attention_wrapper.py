...
from flashinfer import LoadAttention, get_batch_indices_positions, get_seq_lens, append_paged_kv_cache
...
class LoadAttentionWrapper(BaseAttentionWrapper):
    _inst = None
    ...
    def init(
        ...
        self._wrapper = LoadAttention(workspace_buffer, kv_layout="NHD")
        ...
        self.kv_len_arr_tensor = None
        self.batch_indices = None
        self.positions = None
        self.append_seqlen = 0
    ...
    def begin_forward(
        ...
        self.batch_indices, self.positions = get_batch_indices_positions(
            self.qo_indptr, # prefill(bs x seq) + decode(bs x 1)
    ...
    def forward(
        ...
        if self.append_seqlen > 0:
            with self.get_timer(OperationMetrics.ATTN_INPUT_RESHAPE, layer_id):
                append_key = key[: self.append_seqlen].contiguous().reshape(-1, self.num_kv_heads, self.head_dim)
                append_value = value[: self.append_seqlen].contiguous().reshape(-1, self.num_kv_heads, self.head_dim)
        ...
        with self.get_timer(OperationMetrics.ATTN_INPUT_RESHAPE, layer_id):
            query = query[: self.append_seqlen].contiguous().reshape(-1, self.num_q_heads, self.head_dim)

        with self.get_timer(OperationMetrics.ATTN, layer_id):
            pd_output, _ = self._wrapper.run(
                query,
                kv_cache,
        ...
        with self.get_timer(OperationMetrics.ATTN_OUTPUT_RESHAPE, layer_id):
            output[: self.append_seqlen].copy_(
                pd_output.reshape(-1, self.num_q_heads * self.head_dim)
            )
