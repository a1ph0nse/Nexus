...
def torch_attn_with_paged_kvcache(
    ...
    for bi in range(num_batch):
        ...
        output[bi] = Y.transpose(1, 2)

    return output.reshape(num_batch, -1, num_head_q, head_dim)


class TorchAttentionWrapper(BaseAttentionWrapper):
    _inst = None
    ...
    def forward(
        ...
        for prefill_cache_len, prefill_block_table, query_len in zip(
            ...
            with self.get_timer(OperationMetrics.ATTN_PREFILL, layer_id):
                Q_t = seq_query.transpose(1, 2)
                seq_output = torch.empty_like(seq_query)
                seqlen = prefill_cache_len + query_len

                n_blocks = (seqlen + self.block_size - 1) // self.block_size
                BQ = Q_t # [1, h_q, s_q, d]
                blk_ids = prefill_block_table[: n_blocks]
                BK = (
                    kv_cache[0][blk_ids, :, :, :]
                    .view(-1, self.num_kv_heads, self.head_dim)
                    .transpose(0, 1)[:, :seqlen, :]
                    .repeat_interleave(self.num_q_heads // self.num_kv_heads, dim=0)
                ).unsqueeze(0) # [1, h_kv, s_kv, d]
                BV = (
                    kv_cache[1][blk_ids, :, :, :]
                    .view(-1, self.num_kv_heads, self.head_dim)
                ...
                causal_mask = torch.ones(
                query_len, seqlen - query_len, device=BQ.device, dtype=torch.bool
                )
                tail_causal_mask = torch.tril(
                    torch.ones(query_len, query_len, device=BQ.device, dtype=torch.bool)
                )
                causal_mask = torch.cat([causal_mask, tail_causal_mask], dim=-1).unsqueeze(0)
                causal_mask = torch.where(causal_mask, 0.0, float("-inf")).to(BQ.dtype)
                ...
                seq_output = Y.transpose(1, 2)


                seq_output = torch_attn_with_paged_kvcache(
                    seq_query,
                    kv_cache[0],  # k_cache,
                    kv_cache[1],  # v_cache,
    ...
    #         raise e
        ...
        with self.get_timer(OperationMetrics.ATTN_OUTPUT_RESHAPE, layer_id):
            ...
            output[token_offset : token_offset + decode_batch_size].copy_(
                decode_query.reshape(-1, self.num_q_heads * self.head_dim)
            )
